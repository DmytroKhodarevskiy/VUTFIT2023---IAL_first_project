...
void Stack_Init( Stack *stack ) {
	...
	//if stack is NULL, call Stack_Error(SERR_INIT)
	if (stack == NULL){
		Stack_Error(SERR_INIT);
		return;
	}

	//set topIndex to -1 and allocate memory for array
	stack->topIndex = -1;
	stack->array = (char *) malloc(STACK_SIZE * sizeof(char));

	//if allocation failed, call Stack_Error(SERR_INIT)
	if (stack->array == NULL){
		Stack_Error(SERR_INIT);
		return;
	}
...
bool Stack_IsEmpty( const Stack *stack ) {
	...
	//if topIndex is -1, return true, else return false
	if (stack->topIndex == -1)
		return true;
	else
		return false;
...
bool Stack_IsFull( const Stack *stack ) {
	...
	//if topIndex is STACK_SIZE - 1, return true, else return false
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	//if stack is empty, call Stack_Error(SERR_TOP)
	if (Stack_IsEmpty(stack)){
		Stack_Error(SERR_TOP);
		return;
	}

	//set dataPtr to value of topIndex
...
void Stack_Push( Stack *stack, char data ) {
	...
	//if stack is full, call Stack_Error(SERR_PUSH)
	if (Stack_IsFull(stack)){
		Stack_Error(SERR_PUSH);
		return;
	}

	//increment topIndex and set value of topIndex to data
	stack->topIndex++;
...
void Stack_Dispose( Stack *stack ) {
	...
	//if stack is not NULL, free array and set topIndex to -1
	if (stack->array != NULL){
		free(stack->array);	
		stack->array = NULL;
		stack->topIndex = -1;
